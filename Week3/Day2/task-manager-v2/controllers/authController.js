...
export const registerUser = async (req, res) => {
    try {
        ...
        if (userExists) {
            return res.status(400).json({ success: false, message: errors.USER_EXISTS });
        }
...
export const loginUser = async (req, res) => {
    try {
        ...
        if (user && (await bcrypt.compare(password, user.password))) {
            ...
            res.status(401).json({ success: false, message: errors.INVALID_CREDENTIALS });
        }
    } catch (error) {
        res.status(500).json({ success: false, message: errors.SERVER_ERROR });
    }
...
export const getProfile = async (req, res) => {
    try {
        ...
        if (user) {
            res.json({ success: true, message: success.PROFILE_RETRIEVED, data: user });
        } else {
